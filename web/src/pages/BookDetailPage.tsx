...
export function MapMetadataModal({
  book,
  onClose,
  onMapped,
}: {
  book: Book
  onClose: () => void
  onMapped: (book: Book) => void
}) {
  const [query, setQuery] = useState(book.title)
  const [manualId, setManualId] = useState(book.foreignBookId || '')
  ...
  const search = async () => {
    ...
    setError(null)
    try {
      setResults(await api.searchBooks(q))
    } catch (e) {
  ...
  const mapTo = async (foreignBookId: string) => {
    const target = foreignBookId.trim()
    if (!target) return
    setMappingId(target)
    setError(null)
    try {
      const updated = await api.mapBookMetadata(book.id, target)
      onMapped(updated)
      onClose()
    } catch (e) {
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50" onClick={onClose}>
      <div className="bg-slate-100 dark:bg-zinc-900 border border-slate-300 dark:border-zinc-700 rounded-lg w-full max-w-xl shadow-2xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="p-4 border-b border-slate-200 dark:border-zinc-800">
          <h3 className="text-lg font-semibold">Map metadata</h3>
        </div>
        ...
        <div className="p-4 flex-1 overflow-y-auto space-y-4">
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder="OpenLibrary or provider ID"
              className="flex-1 bg-slate-200 dark:bg-zinc-800 border border-slate-300 dark:border-zinc-700 rounded-md px-3 py-2 text-sm focus:outline-none focus:border-emerald-500"
            />
            <button
              onClick={() => mapTo(manualId)}
              disabled={mappingId !== null || !manualId.trim()}
              className="px-4 py-2 bg-slate-700 hover:bg-slate-600 disabled:opacity-50 rounded-md text-sm font-medium"
            >
              {mappingId === manualId.trim() ? 'Mapping…' : 'Map ID'}
            </button>
          </div>
          ...
          <div className="space-y-2 max-h-[45vh] overflow-y-auto">
            {results.map(result => {
              const canMap = !!result.foreignBookId
              const isMapping = mappingId === result.foreignBookId
              return (
                <div key={result.foreignBookId || result.title} className="flex items-center gap-3 p-3 rounded-md bg-slate-200/50 dark:bg-zinc-800/50">
                  {result.imageUrl && <img src={result.imageUrl} alt="" className="w-10 h-14 object-cover rounded flex-shrink-0" />}
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-sm truncate">{result.title}</div>
                    <div className="text-xs text-slate-600 dark:text-zinc-500 truncate">
                      {result.author?.authorName || 'Unknown author'}
                    </div>
                    <div className="text-xs text-slate-500 dark:text-zinc-600 truncate">{result.foreignBookId}</div>
                  </div>
                  <button
                    onClick={() => mapTo(result.foreignBookId)}
                    disabled={!canMap || mappingId !== null}
                    className="px-3 py-1 rounded text-xs font-medium flex-shrink-0 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50"
                  >
