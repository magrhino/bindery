...
- **Calibre library integration** — treat a Calibre library as a first-class storage target, for users who already live in Calibre or want e-reader sync.
  ...
  - [ ] **Configurable per-library mode** — expose both paths as settings so users pick the one that matches their setup. Default to path B (drop folder) because it's the safer option against Calibre schema changes.
